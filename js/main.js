...
import { AudioEngine } from './audio.js?v=1453';
import { createWorld, WORLD } from './world.js?v=1453';
import { buildPlane, makeFighter, applyFlight, forwardOf } from './plane.js?v=1453';
import { buildAAGun, makeAAGun, forwardOfTurret, forwardOfBody, applyAAGun, updateWeakAAGun } from './aagun.js?v=1453';
import { Effects } from './effects.js?v=1453';
import { Net, makePeerId } from './net.js?v=1453';
import { Bot, AAGunBot } from './ai.js?v=1453';

/* ================= 版本号（部署识别；每次发版递增） ================= */
// 注意：不能在此处使用 $（它在文件下部用 const 声明，TDZ 会抛 ReferenceError 导致整个模块崩溃）
const GAME_VERSION = 'v2.3.7';
...
function updateCamera(dt) {
  ...
  if (meIsAAGun) {
    ...
    _look.copy(myPlane.group.position).addScaledVector(_viewDir, 42); _look.y += 2;
    camera.lookAt(_look);
  } else {
    // 死亡：多人观战轨道跟随存活单位；无目标兜底坠机点固定视角
    const st = specTargetObj();
